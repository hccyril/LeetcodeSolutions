...
{
    // medium, 2022/7/9 Daily
    // DP with SortedSet
    internal class P1696跳跃游戏VI
    {
        public const string 相似题 = "1425. 带限制的子序列和"; // but 1425 is hard
        public int MaxResult(int[] nums, int k)
        {
            if (k > nums.Length) k = nums.Length;
            SortedSet<(int s, int i)> sort = new();
            sort.Add((nums[0], 0));
            for (int i = 1; i < k; ++i)
                sort.Add((nums[i] = sort.Max.s + nums[i], i));
            for (int i = 0, j = k; j < nums.Length; ++i, ++j)
            {
                sort.Add((nums[j] = sort.Max.s + nums[j], j));
