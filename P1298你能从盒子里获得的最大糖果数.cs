...
{
    // hard, 2022/5/2
    internal class P1298你能从盒子里获得的最大糖果数
    {
        public int MaxCandies(int[] status, int[] candies, int[][] keys, int[][] containedBoxes, int[] initialBoxes)
        {
            int n = status.Length, ans = 0;
            BitArray boxSet = new(n), keySet = new(n);
            Queue<int> open = new();
            HashSet<int> alreadyOpen = new(); // 第一次提交时没写这个，造成out of memory，原因是某些盒子的钥匙可能有多把
            foreach (var i in initialBoxes)
                if (status[i] == 1) open.Enqueue(i);
                else boxSet[i] = true;
            while (open.Any())
            {
                int bi = open.Dequeue();
                if (!alreadyOpen.Add(bi)) continue;
                ans += candies[bi];
                foreach (var cb in containedBoxes[bi])
                    if (status[cb] == 1) open.Enqueue(cb);
                    else
                    {
                        boxSet[cb] = true;
                        if (keySet[cb]) open.Enqueue(cb);
                    }
                foreach (var kb in keys[bi])
                {
                    keySet[kb] = true;
                    if (boxSet[kb]) open.Enqueue(kb);
                }
            }
            return ans;
        }
