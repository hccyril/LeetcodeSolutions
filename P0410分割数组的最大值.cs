...
namespace ConsoleCore1
{
    ...
    // DP
    // 待做：还可以用二分解法
    internal class P0410分割数组的最大值
    {
        public int SplitArray(int[] nums, int m)
        {
            int[,] dp = new int[m, nums.Length];
            for (int j = 0; j < nums.Length; ++j) 
                dp[0, j] = j == 0 ? nums[0] : dp[0, j - 1] + nums[j];
            for (int i = 1; i < m; ++i)
            {
                for (int j = i; j < nums.Length; ++j)
                {
                    ...
                    {
                        sum += nums[k];
                        min = Math.Max(sum, dp[i - 1, k - 1]);
                        if (min < dp[i, j]) dp[i, j] = min;
                        else if (min > dp[i, j]) break; // 注意==的情况下要继续迭代
                        // else break; 
                    }
                }
            }
